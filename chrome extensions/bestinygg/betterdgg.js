...
;(function(bdgg) {
	bdgg.passivestalk = (function() {
		...
		function updatePassivestalk(psList) {
			...
			bdgg.settings.put('bgg_passive_stalk', psTargets)
			pushChat("Your passivestalk list has been updated")
			document.querySelector('#bdgg_passive_stalk').value = psTargets
		}

		function showUsers(psList) {
			pushChat('Passively stalking the following users: ' + psList.join(', '))
			let chatters = Object.keys(destiny.chat.users).map(property => property.toLowerCase())
			psList = psList.map(x => x.toLowerCase())
			let actives = chatters.filter((val, idx) => {
				return psList.indexOf(val) !== -1
			})
			let inactives = psList.filter((val, idx) => {
				return actives.indexOf(val) === -1
			})
			actives.length === 0 ? pushChat('All users are currently away OnO') : pushChat('These users are currently in chat: ' + actives.join(', '))
			if (inactives.length !== 0 && actives.length !== 0) pushChat('These users are currently away: ' + inactives.join(', '))
		}
		...
		return {
			init: function() {
				...
				destiny.chat.handleCommand = function(str) {
					var match, sendstr
					sendstr = str.trim()
					var psList = bdgg.settings.get('bgg_passive_stalk').split(',')
					...
					if (match = sendstr.match(/^(ps|passivestalk)\s(\w+)/)) {
						...
						(psList.length === 0) ? pushChat('Your passivestalk list is empty'): showUsers(psList)
					} else {
						fnHandleCommand.apply(this, arguments)
					}
				}
...
buf.push("<div id=\"bdgg-about\" style=\"display:none\" class=\"bdgg-menu\"><div class=\"list-wrap clearfix\"></div><div class=\"toolbar\"><h5><span>Bestiny.gg</span><span class=\"close glyphicon glyphicon-remove\"></span></h5></div><h6>v" + (jade.escape((jade_interp = version) == null ? '' : jade_interp)) + "</h6><ul><li>/jn <user> to notify you when <user> enters chat d-(OuO )z</li></ul><hr/><div class=\"bdgg-menu-footer\"><a href=\"https://github.com/0x9ae76c/betterdgg\" target=\"_blank\">Homepage</a></div></div>");}.call(this,"version" in locals_for_with?locals_for_with.version:typeof version!=="undefined"?version:undefined));;return buf.join("");
}
...
;(function(bdgg) {bdgg.version = "1.3.6";}(window.BetterDGG = window.BetterDGG || {}));
